/* PSWARMMEX - A simple MEX Interface to PSWARM Global Solver
...
static void func(int n, int m, double *x, double *lb, double *ub, double *fx);
static double iterfcn(int n, int s, int iter, int gbest, struct swarm *pop);
...
void mexFunction (int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) 
{
    ...
    iterF.enabled = false;
    //Set PSwarm Defaults
    opt.maxf = 10000;       //ten thousand max fvals
    ...
    if(printLevel){            
        ...
        switch((int)*exitflag)
        {
            ...
            case -1:
                mexPrintf("\n *** ERROR: Abnormal Exit ***\n"); break;
            case 0:
                if(stats.solveriters >= opt.maxiter)
        ...
        if(*exitflag==1)
            mexPrintf("\n Final Objective: %12.5g\n In %3.0f iterations and\n   %4d function evaluations\n",*fval,*iter,stats.objfunctions);
...
void func(int n, int m, double *x, double *lb, double *ub, double *fx)
{
    ...
    if(opt.vectorized && mxGetM(fun.plhs[0]) != m)
...
static double iterfcn(int n, int s, int iter, int gbest, struct swarm *pop)
{
    ...
    if(iterF.enabled && iter > 0)
    {
        ...
        //If we are not printing, call drawnow
        //if(printLevel < 2)
...
double getStatus(int stat, int iter, int fevals)
{
    ...
    {        
        case 0:         //normal exit
            if(iter >= opt.maxiter || fevals >= opt.maxf)
                return 0;
