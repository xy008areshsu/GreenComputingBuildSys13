...
if(ncon && ~nlo && ~nlc && ~isNLP)
    ...
    prob = optiprob('f',aslprob.f,'lin',aslprob.A,cl,cu,'bounds',lb,ub,'x0',x0,'sense',sense,'int',int);
    
%Check for QP
elseif(~isempty(aslprob.H) && isempty(aslprob.qcind) && ~isNLP)
    %Build return problem
    prob = optiprob('H',aslprob.H,'f',aslprob.f,'lin',aslprob.A,cl,cu,'bounds',lb,ub,'x0',x0,'sense',sense,'int',int);  
    
%Check for QCQP
elseif(~isempty(aslprob.H) && ~isempty(aslprob.qcind) && ~isNLP)
    %Generate quadratic constraints
    [Q,l,r,ind] = genQC(aslprob);
    %Remove quadratic constraints from linear constraints
    aslprob.A(ind,:) = [];
    cl(ind) = []; cu(ind) = [];
    %Build return problem
    prob = optiprob('H',aslprob.H,'f',aslprob.f,'lin',aslprob.A,cl,cu,'qc',Q,l,r,'bounds',lb,ub,'x0',x0,'sense',sense,'int',int); 
...
if(length(ind) == 1)
    if(xor(isinf(cl(ind)),isinf(cu(ind)))) %single sided constraint [1 con]
        if(isinf(cl(ind)))
            Q = prob.Q{1};
            l = prob.l;                
            r = cu(ind);                 
        ...
        %Create one constraint for each side
        Q = {prob.Q{1}; -prob.Q{1}};
        l = [prob.l -prob.l];
        r = [prob.cu(ind) -prob.cl(ind)];
    end
%Multiple constraints
else
    %Determine no of resulting QCs
    nqc = length(ind) + sum(~xor(isinf(cl(ind)),isinf(cu(ind)))); indQC=1;
    %Preallocate
    Q = cell(nqc,1); l = zeros(size(prob.l,1),nqc); r = zeros(1,nqc);
    for i = 1:length(ind)
        qci = ind(i);
        if(xor(isinf(cl(qci)),isinf(cu(qci)))) %single sided constraint 
            if(isinf(cl(ind)))
                Q(indQC) = prob.Q(i); 
                l(:,indQC) = prob.l(:,i);                
                r(indQC) = cu(qci);                 
            ...
            Q(indQC) = prob.Q(i);
            Q(indQC+1) = {pQ};
            l(:,indQC:indQC+1) = [prob.l(:,i) -prob.l(:,i)];
            r(indQC:indQC+1) = [cu(qci) -cl(qci)];
            indQC = indQC + 2;
        end       
    end
end
