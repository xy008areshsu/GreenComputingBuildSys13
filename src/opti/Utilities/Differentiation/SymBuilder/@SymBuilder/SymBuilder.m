...
%   Copyright (C) 2012 Jonathan Currie (www.i2c2.aut.ac.nz)
    ...
    methods
        ...
        %Get Linear Constraint Function [rl <= A*x <= ru]
            ...
            %Get Linear Part & Indices
            ind = B.conlin == 1;   
            if(sum(ind) == 0)
                error('This object does not contain linear constraints');
            end
        ...
        %Get Nonlinear Constraint Function [cl <= c(x) <= cu]
            ...
            ind = B.conlin >= 2; 
            if(sum(ind) == 0)
                error('This object does not contain nonlinear constraints');
            end
            nljac = B.jac(ind,:);
            %Build NL Con Callback + row constraints
        ...
        %Get all constraints as nonlinear callback function
            ...
            if(isempty(B.conlin))
                error('This object does not contain any constraints');
            end
            ind = B.conlin > 0;
            ajac = B.jac(ind,:);
        ...
        %Solve Object using OPTI
            ...
            if(nargin < 3), opts = []; end
            if(nargin < 2), x0 = []; end
            %Build OPTI Object if empty
            if(isempty(B.Opt))
                fprintf('\nGenerating OPTI Object....\n');
                if(isfield(opts,'solver') && strcmpi(opts.solver,'scip'))
                    B.Opt = GetOPTI(B,'scip'); %scip creates its own derivatives
                else
                    B.Opt = GetOPTI(B);
                end
                fprintf('Done\n\n');
            ...
            %Assign Options
            if(~isempty(opts))
                o = B.Opt.opts; %get old options, should only ever contain solverOpts
                if(~isempty(o.solverOpts))                   
                    switch(o.solver)
                        case 'bonmin'
                            opts = optiset(opts,'solverOpts',bonminset(opts.solverOpts,'var_lin',o.solverOpts.var_lin,'cons_lin',o.solverOpts.cons_lin));
                        case 'scip'
                            %nothing to do
                        otherwise
                            error('not configured to accept solverOpts other than for bonmin');
                    end                      
                end
                B.Opt = opti(B.Opt,'options',opts);
            end
            %Solve
